...
export class Agents extends BaseResource {
  async list({ options }: { options?: RequestOptions } = {}): Promise<Agent[]> {
    return this.client.get<Agent[]>("/agents", options);
  }

  async retrieve({ id, options }: { id: string; options?: RequestOptions }): Promise<Agent> {
  ...
  async create({
    ...
    payload: CreateBetaAgentPayload;
  ...
  async clone({ id, options }: { id: string; options?: RequestOptions }): Promise<Agent> {
